...
console.log('Tentative de chargement des credentials depuis:', CREDENTIALS_PATH);
...
router.get('/callback', async (req, res) => {
  ...
  if (!code) {
    console.error('Pas de code dans la callback');
  ...
  try {
    ...
    // Redirection vers le frontend avec un paramètre de succès
    const redirectUrl = new URL('/dashboard/mail', process.env.FRONTEND_URL || 'http://localhost:5173');
